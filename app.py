...
def upload_file():
    ...
    if upload_file_type != "HTML":
        ...
        filename = generate_unique_filename(filename, chosen_bbs, 22)
...
def generate_unique_filename(filename, chosen_bbs, cutting_length):
    global sid_data
    base, ext = os.path.splitext(filename)
    
    # If the filename (including its extension) is longer than 11 characters, truncate it.
    if len(filename) > cutting_length:
        base = base[:cutting_length-len(ext)]  # Adjust so that the total length remains 11 characters
    
    original_base = base
    counter = 1

    # Check if file exists and modify the filename accordingly
    filename_pattern = re.compile("^" + re.escape(base + ext) + "$", re.IGNORECASE)
    while uploads_collection.find_one({"filename": filename_pattern, "chosen_bbs" : chosen_bbs}):
        # Adjust the base name to accommodate the counter
        base = original_base[:cutting_length-len(ext)-len(str(counter))] + str(counter)
        counter += 1
