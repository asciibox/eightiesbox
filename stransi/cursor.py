...
class CursorMove:
    ...
    @staticmethod
    def to(x: int = 0, y: int = 0, relative = False) -> CursorMove:
        """
        ...
        return CursorMove(x=x, y=y,relative=relative)
...
@dataclass
class SaveCursor(Instruction):
    """Instruction to save the current cursor position."""
    pass

@dataclass
class RestoreCursor(Instruction):
    """Instruction to restore the cursor to the previously saved position."""
    pass
