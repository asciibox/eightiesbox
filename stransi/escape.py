...
from .cursor import CursorMove, SetCursor, SaveCursor, RestoreCursor
        ...
        while token := next(tokens, None):
            ...
            if token.kind in {"H", "f"}:
                try:
                    ...
                    next_data = 0
                y = token.data if token.data else 1
                x = next_data if next_data else 1
                # ANSI escape sequences are 1-based, but we want 0-based.
                yield SetCursor(CursorMove.to(x - 1, y - 1, relative = False))
                continue
