UI.modalDialog = function (initialProperties) {
  ...
  me.setProperties = function (p) {
    ...
    background.setPosition(
      Math.floor((me.width - panelWidth) / 2),
      Math.floor((me.height - panelHeight) / 2)
    );

    if (me.cancel) {
      okButton.setPosition(
        background.left + Math.floor(background.width / 2) - 110,
        background.top + background.height - 40
      );
      cancelButton.setPosition(
        background.left + Math.floor(background.width / 2) + 10,
        background.top + background.height - 40
      );
    } else {
      okButton.setPosition(
        background.left + Math.floor(background.width / 2) - 50,
        background.top + background.height - 40
      );
    }
  ...
  var background = UI.scale9Panel(
    0,
    0,
    Math.floor(me.width / 2),
    200,
    UI.Assets.panelMainScale9
  );
      ...
      if (text) {
        ...
        lines.forEach(function (line) {
          ...
          if (fontFT) {
            ...
            textX =
              background.left + 10 + Math.floor((maxWidth - textLength) / 2);
